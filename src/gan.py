...
def model_generator():
    ...
    fake_label = Input(shape=fake_label_shape, name='fake_label')
    concat = Concatenate()([input_latent, fake_label])
    ...
    fc_last = Dense(30, activation='linear', name='fc_last')(fc_4)

    generator = Model(inputs=[input_latent, fake_label], outputs=fc_last)
...
def model_discriminator():
    latent_shape = (30,)
    input_latent = Input(shape=latent_shape, name='input_latent')
    fc_1 = Dense(100, activation='elu', name='fc_1')(input_latent)
    fc_2 = Dense(1000, activation='elu', name='fc_2')(fc_1)
    fc_3 = Dense(1000, activation='elu', name='fc_3')(fc_2)
    fc_4 = Dense(100, activation='elu', name='fc_4')(fc_3)
    fc_last = Dense(2, activation='softmax', name='fc_last')(fc_4)

    discriminator = Model(inputs=input_latent, outputs=fc_last)
...
def create_gan(discriminator, generator):
    ...
    input_latent = (30,)
    fake_label = (2,)
    x = generator((input_latent, fake_label))
    gan_output= discriminator(x)
    
    gan= Model(inputs=[input_latent, fake_label], outputs=gan_output)
    gan.compile(loss='crossentropy', optimizer='adam')
    return gan
